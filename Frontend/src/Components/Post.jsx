...
function Post({post}) {
    ...
    const deletePostHandler = async(e) =>{
      try{
        const res = await axios.delete(`https://luka-social-media-application.onrender.com/api/v1/posts/delete/${post._id}` , {withCredentials:true})
    ...
    const commentHandler = async () =>{
      try{
       const res = await axios.post(`https://luka-social-media-application.onrender.com/api/v1/posts/${post._id}/comment`,{text},{headers:{"Content-Type":'application/json'},withCredentials:true});
       if(res.data.success){
        toast.success(res.data.message);
    ...
    const bookmarkHandler = async(postId) =>{
      try{
        const res = await axios.get(`https://luka-social-media-application.onrender.com/api/v1/posts/${postId}/bookmark`,{headers:{"Content-Type":'application/json'}, withCredentials:true});
        // console.log(res.data);
        if(res.data.success){
          toast.success(res.data.message);
        }
